...
但是对于网站而言，分享将会收到很大的限制，比如对于 **微信分享** 而言，

在普通网站应用中将无法进行对接。所以我们本章中只能针对于 **微博** 实现分享功能
...
但是对于微信而言，它不支持普通网站的分享，仅支持:` APP` 或 微信公众号、企业号、服务号 的分享内容

**微博**

微博的分享原理与微信一样：**把一段信息或图片，发送到微博中**

总结：综合起来来说，所谓的分享指的就是：**把一段内容直接发布到对应的平台之中**
...
1. 进入 [**新浪开发平台**](https://open.weibo.com/)
2. 点击 登录 按钮

**新注册用户（企业级、应用级）**

1. 点击立即注册
2. 通常情况下，企业级的微博分享会有专门的账号,即：**一个应用一个账号。**
3. 个人注册和官方注册。（选官方注册-应用）
4. 短信验证
5. 完成注册

**个人登录（练习使用）**

使用现有手机号登录，再转化成开发者的方式

登录之后，右击个人头像->开发者信息。新建开发者。

**完成身份认证**

重新进入 **开发者信息页面**，选择 **身份认证** -> 完成认证等待审核通过 -> 进入创建应用-> 完善用户信息等待审核。

### 20.4 微博分享对接实现

整个微博的分享对接非常简单，我们知道所谓分享只是将一段内容发布到对应平台，

所以我们只需要将这段内容填充到发布的` url `中即可。

1. 在 `index.html` 中，倒入微博分享的 `js SDK` 
...
3. 页面增加分享点击事件： `src\views\main\components\list\item.vue`
...
**注册认证支付宝企业账号（个人无法使用支付功能）**

1. 打开 **[支付宝开放平台](https://open.alipay.com/)**
2. 点击 免费注册，进入注册页面说明（用商家注册）
3. 开始填写主体信息
4. 经营场景先选择：电脑网站支付，手机网站支付
5. 完善账户信息（对公账户打款验证）
6. 绑定邮箱，设置登录密码、支付密码
7. 进入 **商家中心**
...
在我们注册、认证了企业账号之后，此时在 支付宝开放平台，通过 **绑定邮箱 **和 **登录密码** 进行登录

立即入住开放平台、先写开发对接的信息

在控制台 **创建应用** **网页移动应用**

应用创建完成之后，我们需要为应用添加 **手机网站支付** 和 **电脑网站支付** 的能力

点击 **添加能力** 按钮 -> 支付 添加2个支付 -> 关联能力（关联商家号）

商家中心 -> `appid绑定`-> 添加绑定-> 完善信息->提交

手机网站支付需要等到应用上线之后才能生效。

**上线应用**

期望完成 支付宝支付 功能对接之前，你需要先完成 应用的上线（直接点击上线即可）。否则你可能会得到一个` ISV` 的错误。

### 21.4 支付宝支付对接-支付功能技术对接——PC端支付

1. 集成并配置SDK：支付宝支付对接的整体文档
2. 电脑网站支付接入助手：对接的向导
3. 咨询服务：人工技术咨询、答疑服务
...
定义工具类，触发接口，触发支付功能：`src\utils\pay.js`
...
export const alipay = async (title, desc) => {
  ...
  // 解构 编码过得 支付宝支付路径 encodeURI
  window.location.href = decodeURIComponent(encodeURI)
}
```

支付页面调用：`src\views\member\components\payment\pc-payment\index.vue`
...
创建支付宝回调支付组件：`src\views\pay\index.vue`
...
路由增加PC端回调路由：`src\router\modules\pc-routes.js`

移动端也需要添加：`src\router\modules\mobile-routes.js`
...
移动端支付核心的关键：`src\utils\flexible.js`
...
移动端支付按钮事件：`src\views\member\components\payment\mobile-payment\mobile-pay-select.vue`
...
用草料二维码把网址首页放进去生成移动端二维码，用支付宝扫二维码。然后支付
...
8. 用户在该页面完成支付，支付完成之后，支付宝会回调两个地址:
   ...
   2. `notifyUrl`：异步通知地址，以`http`或者`https`开头的，商户外网可以post访问的异步地址，用于接收支付宝返回的支付结果
...
之前在对接 微信登录 时，我们说过，在微信开放平台的应用列表中：

虽然有微信支付 但是并不可用。

截止到 2022年 维系支付并**不支持 PC网站对接微信支付功能**，当申请完成 **商户号** 和 **网站应用** 之后，使用 商户号对网站应用授权时，会得到这样一个提示：**“当前商户号暂不支持关联该类型的`AppID`”**。
...
对于项目打包而言它的一个功能是通用的功能，大家也可以理解为任何一个项目都是一样的。

并且对于大多数的同学而言，应该使用的都是 windows 系统，所以我这里将使用上一门课程中通过 windows 录制的内容，作为咱们这次课程的部署视频。

同时我也把咱们课程最终的 `nginx` 配置配置放入到了咱们的图文节中，

并且由详细的备注，相信大家结合视频和实际的配置，完全可以部署自己的项目到服务端

### 22.2 域名、DNS、公网IP、服务器、Nginx之间的关系

在处理我们的服务之前，我们明确一些基本的概念，这些概念有:

1. 域名：https://imooc-admin.lgdsunday.club，腾讯云购买的
2. `DNS`：域名解析服务器
3. 公网`IP`：服务器在网络中的唯一地址
4. 服务器：服务部署的电脑
5. `Nginx`：网页服务

电脑 -> 浏览器`www` -> `DNS` -> `IP` -> 服务器 -> `NGINX`-> 服务

### 22.3 阿里云服务器购买指南

云服务器`ECS`自定义购买：[**阿里云服务器`ECS`**](https://www.aliyun.com/product/ecs?spm=5176.29741907.J_gyAH7ReF7dbGhIsHQpTqj.d_0_0.9972154apXBsmx&scm=20140722.M_ecs.P_186.ID_ecs-OR_rec-V_1-MO_3127-ST_10667)

付费模式：一般包年包月



自己玩服务器：购买一个 `1vCPU`（一核CPU）1`GiB`内存。

公司级服务器：至少需要`2vCPU`，4`Gib`内存。

架构：`X86`计算机，

镜像：指服务器安装的系统，`CentOS(6.9,7.2,7.4)`、`WindowServe`、`AilIbaba`

硬盘：随便，云`ESSD` 40G

共享标准型，

分配公网`IPV4` ,按固定带宽, 选穷人`1M`带宽

选填可以不填。直接确认订单。大概1000块钱。
...
这里通过 第二种方式 SSH 工具（`XShell`）进行连接

 新建会话,协议选择SSH，输入`IP`（阿里云自动分配的），端口默认22

左侧双击，就会要求输入用户名和密码。默认：root，密码
...
13. 查看 `nginx` 配置
    ...
    # 当前输入 nginx -t 来检查是不行的，必须输入上面完整的路径。要想直接nginx -t 查看信息，需要制作软连接，如下步骤:
    ```
...
27. 通过 nginx -s reload 重启服务
    ...
    ```bash
    # nginx -s reload 没有办法直接启动服务的话，可以执行下面命令,指定路径
    nginx -t
    nginx -c /usr/local/nginx/conf/nginx.conf # nginx 根据这个配置文件来启动
    nginx -s reload
    ```
...
28. 在浏览器中通过 `IP`测试访问
    ...
    ```bash
    浏览器输入：127.0.0.1  看看页面是否有内容
    ```

### 22.6  项目发布

可以通过 **[`XFTP`](https://www.xshell.com/zh/)** 进行数据传输，和 **`XShell`** 是一家的

把打包后的静态资源放在 **`/nginx/dist`** 下面
